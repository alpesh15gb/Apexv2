# Apex HRMS - Business Workflow Validation Report

> Generated from source code analysis of `backend/app/api/v1/endpoints/` and `backend/app/api/v1/endpoints/school/`
...
**Source:** `backend/app/api/v1/endpoints/auth.py:46`, `backend/app/api/v1/endpoints/tenants.py`
...
#### Workflow B: Superuser Creates Tenant
...
| Status | Condition |
|--------|-----------|
| 403 | Non-superuser attempts: Permission denied |
| 401 | Missing or invalid token |

#### Workflow C: List All Tenants (Superuser)
...
After tenant creation, configure tenant-specific settings:

```
GET  /api/v1/tenant-settings/     # Read current settings
...
| Status | Condition |
|--------|-----------|
| 404 | Department/Designation/Branch not found |
| 400 | Duplicate employee code or email |
...
```
POST /api/v1/lifecycle/{employee_id}/promote
Authorization: Bearer <token>
```
...
3. Creates `EmployeeEvent` record with type `"promotion"`
4. Commits transaction

**Error Handling:**
| Status | Condition |
|--------|-----------|
| 404 | Employee not found or belongs to different tenant |

#### Step 5: Transfer

```
POST /api/v1/lifecycle/{employee_id}/transfer
Authorization: Bearer <token>
```
...
**Data Flow:**
1. Updates `employee.department_id` if `new_department_id` provided
2. Updates `employee.branch_id` if `new_branch_id` provided
3. Creates `EmployeeEvent` with type `"transfer"`

#### Step 6: Resignation

```
POST /api/v1/lifecycle/{employee_id}/resign
Authorization: Bearer <token>
```
...
```
POST /api/v1/lifecycle/{employee_id}/confirm
```

**Terminate:**
```
POST /api/v1/lifecycle/{employee_id}/terminate
```
Sets `employee.status = "terminated"`.

**Reactivate:**
```
POST /api/v1/lifecycle/{employee_id}/reactivate
```
Sets `employee.status = "active"`.

**Salary Revision:**
```
POST /api/v1/lifecycle/{employee_id}/salary-revision
```

#### View Employee Timeline

```
GET /api/v1/lifecycle/{employee_id}/timeline
Authorization: Bearer <token>
```
...
Confirm from Probation → EmployeeEvent(confirmation)
     ↓
Promotion → designation updated + EmployeeEvent(promotion)
     ↓
...
| 400 | Duplicate attendance for same date |
...
> Requires `payroll` feature enabled on tenant.
...
**Source:** `backend/app/api/v1/endpoints/auth.py:46`

School tenants use the same registration endpoint as corporate tenants. The school-specific features are enabled via tenant feature flags.
...
```json
{
  "tenant_name": "Sunrise International School",
  "tenant_slug": "sunrise-school",
  "admin_email": "admin@sunriseschool.edu",
  "admin_password": "SchoolAdmin2025!",
  "admin_full_name": "Dr. Principal Name"
}
```
...
2. Creates default RBAC roles (Super Admin, Admin, Teacher, etc.)
3. Creates admin user with Super Admin role
4. School features are configured via tenant settings post-registration

**Required Features to Enable:**
- `academic_year`
- `class_management`
- `student_management`
- `student_attendance`
...
| 400 | Application not in enrollable state |
...
```
GET /api/v1/school/students/{student_id}/guardians
```
...
All endpoints are prefixed with `/api/v1/`. The school module endpoints are under `/api/v1/school/`.
...
| Examinations | `exam.read` | `exam.manage` |
