      ...
      body: docsAsync.when(
        data: (docs) {
          if (docs.isEmpty) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.folder_open, size: 48, color: ApexColors.neutral400),
            const SizedBox(height: 16),
            Text('No Documents', style: ApexTypography.headingMedium.copyWith(color: ApexColors.neutral900)),
            const SizedBox(height: 8),
            Text('Upload documents to get started', style: ApexTypography.body.copyWith(color: ApexColors.neutral500)),
          ]));
          return ListView.builder(padding: const EdgeInsets.all(16), itemCount: docs.length, itemBuilder: (context, i) {
            ...
            return Padding(
              ...
              child: ApexCard(
                ...
                child: Row(children: [
                  Container(width: 40, height: 40, decoration: BoxDecoration(color: ApexColors.primary600.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: Icon(typeIcons[d.docType] ?? Icons.insert_drive_file, color: ApexColors.primary600, size: 20)),
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(children: [Expanded(child: Text(d.title, style: ApexTypography.titleSmall.copyWith(color: ApexColors.neutral900))), if (d.isConfidential) Icon(Icons.lock, size: 14, color: ApexColors.error)]),
                    Text('${d.docType.replaceAll("_", " ").toUpperCase()} • ${d.fileName}', style: ApexTypography.caption.copyWith(color: ApexColors.neutral500)),
                  ])),
                  IconButton(icon: Icon(Icons.delete_outline, size: 18, color: ApexColors.error), onPressed: () => ref.read(documentListProvider.notifier).delete(d.id)),
                ]),
              ),
            );
          });
  ...
  void _showAddDialog(BuildContext context, WidgetRef ref) {
    ...
    showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => AlertDialog(
      ...
      content: SizedBox(width: 400, child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        ApexDropdown<String>(label: 'Type', value: docType, items: const [
      ...
      actions: [
        ...
        ApexButton(label: 'Add', onPressed: () async {
