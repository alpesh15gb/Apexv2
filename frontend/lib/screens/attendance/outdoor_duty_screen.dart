...
class OutdoorDutyScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: const ApexAppBar(title: 'Outdoor Duty'),
      body: odAsync.when(
        data: (items) {
          if (items.isEmpty) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.directions_walk, size: 48, color: ApexColors.neutral500),
            const SizedBox(height: 16),
            Text('No Outdoor Duty Entries', style: ApexTypography.headingMedium.copyWith(color: ApexColors.neutral900)),
            const SizedBox(height: 8),
            Text('OD entries will appear here when created', style: ApexTypography.body.copyWith(color: ApexColors.neutral500)),
          ]));
          return ListView.builder(padding: const EdgeInsets.all(16), itemCount: items.length, itemBuilder: (context, i) {
            ...
            return Padding(
              ...
              badge,
              if (o.status == 'pending') ...[
                IconButton(icon: Icon(Icons.check, size: 18, color: ApexColors.success), onPressed: () => ref.read(odListProvider.notifier).approve(o.id)),
                IconButton(icon: Icon(Icons.close, size: 18, color: ApexColors.error), onPressed: () => ref.read(odListProvider.notifier).reject(o.id)),
              ],
            ]),
            ),
            );
