...
class OTRegisterScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: const ApexAppBar(title: 'OT Register'),
      body: otAsync.when(
        data: (items) {
          if (items.isEmpty) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.access_time_filled, size: 48, color: ApexColors.neutral500),
            const SizedBox(height: 16),
            Text('No OT Records', style: ApexTypography.headingMedium.copyWith(color: ApexColors.neutral900)),
            const SizedBox(height: 8),
            Text('Overtime records will appear here', style: ApexTypography.body.copyWith(color: ApexColors.neutral500)),
          ]));
          return ListView.builder(padding: const EdgeInsets.all(16), itemCount: items.length, itemBuilder: (context, i) {
            ...
            return Padding(
              ...
              badge,
              if (o.status == 'pending') ...[
                IconButton(icon: Icon(Icons.check, size: 18, color: ApexColors.success), onPressed: () => ref.read(otListProvider.notifier).approve(o.id)),
                IconButton(icon: Icon(Icons.lock, size: 18, color: ApexColors.primary), onPressed: () => ref.read(otListProvider.notifier).preserve(o.id)),
              ],
            ]),
            ),
            );
